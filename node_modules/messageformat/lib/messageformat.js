...
class MessageFormat {
  ...
  constructor(locale) {
    ...
    if (typeof locale === 'string') {
      this.pluralFuncs[locale] = (0, _plurals.getPlural)(locale);
      this.defaultLocale = locale;
    } else if (Array.isArray(locale)) {
      locale.forEach(lc => {
        this.pluralFuncs[lc] = (0, _plurals.getPlural)(lc);
      });
      this.defaultLocale = locale[0];
    } else {
      for (const lc in locale) if (locale.hasOwnProperty(lc)) {
  ...
  addFormatters(fmt) {
    for (const name in fmt) if (fmt.hasOwnProperty(name)) this.fmt[name] = fmt[name];
  ...
  compile(messages, locale) {
    ...
    if (Object.keys(this.pluralFuncs).length === 0) {
      if (locale) {
        const pfn0 = (0, _plurals.getPlural)(locale, this.noPluralKeyChecks);
        ...
        pf = (0, _plurals.getAllPlurals)(this.noPluralKeyChecks);
      }
    } else if (locale) {
